export class State {
  ...
  #update(e) {
    ...
    updatedKeys.forEach(key => {
      ...
      // Notify parent object watcher if a property changed
      // TODO: fires twice for nested updates like 'user.name', once for 'user.name' and once for 'user'.
      //       So, a watcher of 'user' will be called twice if 'user.name' is updated.
      if (key.includes('.')) {
        const parentKey = key.split('.').slice(0, -1).join('.');
        const parentCbs = this.#watchers.get(parentKey);
        if (parentCbs) {
          // Get the parent object
          const parentValue = parentKey ? parentKey.split('.').reduce((o, k) => o?.[k], this.#values) : this.#values;
