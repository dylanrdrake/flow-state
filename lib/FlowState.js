export class FlowState {
  ...
  #updateListBindingsForKey(key, value) {
    const containers = this.#getDirectiveBindingsFromRoot(`[flow-list="${key}"]`, this.#root);
      ...
      fragment.querySelectorAll('*').forEach(el => {
        for (const { name, value } of [...el.attributes]) {
          if (name.startsWith('flow-') && !name.startsWith('flow-watch-') && name.endsWith('-to-prop')) {
            ...
            const itemValue = this.#getValueByDashPathCaseInsensitive(item, rawKey);
  ...
  // HTML lowercases attribute names, so flow-list key segments may arrive lowercased.
  ...
  static watch(element, key, callback) {
    return watchFlowFrom(element, key, callback);
  }


  static get(element, key) {
    return getFlowFrom(element, key);
  }


  static through(shadowRoot) {
    return flowThrough(shadowRoot);
  }
  ...
  static create(root, config = {}) {
    return createFlowFrom(root, config);
  }
...
export function createFlowFrom(root, config = {}) {
  if (root instanceof Node) {
    return new FlowState(root, config);
  }
  throw Error('createFlowFrom requires the root to be a DOM Node!');
}


export function getFlowFrom(source, key) {
  if (isFlowLike(source)) {
...
export function watchFlowFrom(source, key, callback) {
  if (isFlowLike(source)) {
    return source.watch(key, callback);
  }
...
export function flowCompute(fn, deps = []) {
  return FlowState.compute(fn, deps);
}


export function startFlowDevtools() {
  return FlowState.startFlowDevtools();
}