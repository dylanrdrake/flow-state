...
export function flowCompute(fn, deps = []) {
  ...
  const DEVTOOLS_WINDOW_NAME = 'flowstate-devtools';
  const targetName = window.name === DEVTOOLS_WINDOW_NAME ? '_blank' : DEVTOOLS_WINDOW_NAME;
  const tab = window.open(url.href, targetName);
  if (tab && targetName === '_blank') {
    try {
      tab.name = DEVTOOLS_WINDOW_NAME;
    } catch {
      // Ignore failures when assigning window name.
    }
  }
  if (!tab || !tab.closed) {
