...
export class FlowState {
  ...
  constructor(root, config = {}) {
    ...
    // Parse flat config: actions key → actions, functions → computed, rest → state values
    const actions = {};
    ...
    for (const [k, v] of Object.entries(config ?? {})) {
      if (k === 'actions') {
        Object.assign(actions, v ?? {});
      } else if (typeof v === 'function') {
        computedEntries.push([k, v]);
      } else {
  ...
  async #update(e, notifyWatchers = false) {
    ...
    // Update bindings — list bindings first so newly rendered items are in the DOM
    // before the prop/attr pass runs (items may carry flow-watch-* bindings on other keys)
    allKeysToUpdate.forEach(key => {
      const value = this.#computed[key]
        ? this.#computed[key](this.#values)
  ...
  //   flow-item-to-prop="itemKey:domProp"   — sets el[domProp] = item[itemKey]
  //   flow-item-to-attr="itemKey:attrName"  — sets el.setAttribute(attrName, item[itemKey])
  #renderList(container, items) {
    ...
    items.forEach(item => {
      ...
      fragment.querySelectorAll('[flow-item-to-prop]').forEach(el => {
        const [itemKey, domProp] = el.getAttribute('flow-item-to-prop').split(':');
        if (itemKey && domProp && itemKey in item) {
          el[domProp] = item[itemKey];
        }
      });

      fragment.querySelectorAll('[flow-item-to-attr]').forEach(el => {
        const [itemKey, attrName] = el.getAttribute('flow-item-to-attr').split(':');
        if (itemKey && attrName && itemKey in item) {
          el.setAttribute(attrName, String(item[itemKey]));
        }
      });
