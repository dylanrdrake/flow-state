export class FlowState {
  ...
  #getBindingsForKey(key) {
    ...
    const propSelector = `[flow-bind-${dashKey}-to-prop]`;
    const attrSelector = `[flow-bind-${dashKey}-to-attr]`;
  ...
  // the root for elements with matching flow-bind attributes
  #updateBindingsForKey(key) {
    ...
    propBindings.forEach(el => {
      ...
      const prop = el.getAttribute(`flow-bind-${dashKey}-to-prop`);
      if (prop) {
        el[prop] = value;
      }
    })
    ...
    attrBindings.forEach(el => {
      ...
      const attr = el.getAttribute(`flow-bind-${dashKey}-to-attr`);
      if (attr) {
