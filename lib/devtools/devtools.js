...
export function startFlowStateDevtools() {
  ...
  const getExpandedInstancesSet = () => new Set(flowGet(appRoot, 'expandedInstances') || []);
  const setExpandedInstancesSet = (setVal) => state.update({ expandedInstances: Array.from(setVal) });
  ...
  function renderTree() {
    ...
    const expandedInstances = getExpandedInstancesSet();
    ...
    function renderNode(id, depth) {
      ...
      if (toggleEl && nodeChildren.length > 0) {
        ...
        toggleEl.addEventListener('click', (e) => {
          ...
          setExpandedInstancesSet(expandedInstances);
        });
      }
    ...
    entries.forEach(([id, p]) => {
      ...
      g.addEventListener('click', () => {
        ...
        if (nodeChildren.length > 0) {
          const expandedInstances = getExpandedInstancesSet();
          expandedInstances.add(id);
          patch.expandedInstances = Array.from(expandedInstances);
        }
        state.update(patch);
      });
  ...
  watch('expandedInstances', () => renderFromState());
  ...
  const onChannelMessage = (e) => {
    ...
    if (isNew) {
      const expanded = new Set(flowGet(appRoot, 'expandedInstances') || []);
      expanded.add(snap.id);
      if (snap.parentId) expanded.add(snap.parentId);
      state.update({ expandedInstances: Array.from(expanded) });
    }
