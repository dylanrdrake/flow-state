...
export function createDevtoolsRenderers({ channel, graphPanel, store }) {
  ...
  const getNodeName = (snap) => snap.hostClassName || snap.label || snap.rootTag;
  ...
  function renderTree() {
    ...
    function renderNode(id, depth) {
      ...
      const tagHtml = `${getNodeName(snap)} &lt;${snap.rootTag}&gt;`;
      const nodeChildren = children.get(id) ?? [];
      const isExpanded = expandedSources.has(id);
      const keyCount = snap.values ? Object.keys(snap.values).length : 0;
      const actionCount = Array.isArray(snap.actionKeys) ? snap.actionKeys.length : 0;
      ...
      rowEl.innerHTML = `
        ...
        <span class="tree-badges">
          ...
          ${actionCount > 0 ? `<span class="badge action">${actionCount}a</span>` : ''}
      ...
      const toggleEl = /** @type {HTMLElement | null} */ (rowEl.querySelector('.tree-toggle'));
  ...
  function renderGraph() {
    ...
    entries.forEach(([id, p]) => {
      ...
      const keyCount = snap.values ? Object.keys(snap.values).length : 0;
      const actionCount = Array.isArray(snap.actionKeys) ? snap.actionKeys.length : 0;
      stats.textContent = `${keyCount}k  ${snap.watcherCount || 0}w  ${snap.computedKeys.length || 0}c${actionCount > 0 ? `  ${actionCount}a` : ''}`;
      g.appendChild(stats);
