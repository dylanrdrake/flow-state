...
function resolveTargetPath(urlPathname) {
  const subPath = urlPathname.replace(/^\/target/, '') || '/';
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/') {
    res.writeHead(302, { Location: `${TARGET_BASE}/` });
    res.end();
    return;
  }
  ...
  if (url.pathname === TARGET_BASE) {
    res.writeHead(302, { Location: `${TARGET_BASE}/` });
    res.end();
    return;
  }

  if (url.pathname.startsWith(`${TARGET_BASE}/`)) {
...
server.listen(PORT, () => {
  ...
  console.log(`Target root (${STATIC_ROOT}) -> http://localhost:${PORT}${TARGET_BASE}/`);
});
